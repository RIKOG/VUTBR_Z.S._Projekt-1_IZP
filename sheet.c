// TODO NEFUNGUJE DCOLS 4 4 AJ KED NEUSKUTOCNI, POVIE ZE ODSTRANILO STLPEC CO JE PICOVINA
//Richard Gajdosik, IDE = CLION
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
// dlzku tabulky vzdy upravujem vo funkciach, pocet stlpcov upravujem v maine
// todo 3. vstup ked narazí na EOF skončiť program
// todo když mám int, a jak zaokrouhlíš třeba "boris", nijak, tak to jde dál
// todo cokolvek so str aj tak funguje napriek limitacii na 101 lmao
...
// todo rewrite round, int -  Pokud zavolám funkci round (nebo int) a sloupec je prázdný, mám tam vložit nulu nebo to nechat prázdný?
#define MAX 10241
int koniec_riadku = 0;
...
int main(int argc, char *argv[]) {
    char delimiter = ' ', tabulka[MAX] = {0}, STR[101] = {0}, STR_2[101] = {0}, delimiter_array[MAX] = {0};
    ...
    char argument_contains[MAX] = "contains";
    while (!koniec_riadku) {
        i = 1;
        while (i < argc) {
            if ((strcmp(argv[i], "-d")) == 0) {
                strcpy(delimiter_array, argv[++i]);
                if((int)strlen(argv[i]) > 100){ //todo chybnu hlasku plus magicke cislo
                    return -1;
                }
...
//                    printf("\nYOU done fuckedup\n\n");
                    tabulka[0] = '\0';
                    koniec_riadku = 1;
                    break; //todo chcem skoncit kompletne vsetko, vypisat error presiahnutie miery, cez flag
                }
                ...
                if (delimiter != 0 && tabulka[0] != '\0' && command_riadok > 0 && command_riadok == riadok && rows == 1) { // ak sa nachadzame na riadku, pred ktorý chceme dat riadok
                ...
                if (delimiter != 0 && tabulka[0] != '\0' && rows == 1) { // todo delimiter_array[0] != '\0'
...
//                    printf("%d\n", rows_command_riadok);
                ...
                command_stlpec = atoi(argv[++i]);
                strcpy(STR, argv[++i]);
                if(command_stlpec <= 0 || (int)strlen(argv[i]) > 100){
                    return -1;
                }
